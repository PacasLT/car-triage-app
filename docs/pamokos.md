# Pamokos ir istorija · CarTriige

Iki 2026-09-22 tai buvo `CLAUDE.md` (494 eil.). Perkelta **nepakeista** (DARBO-SISTEMA A6),
kad `CLAUDE.md` liktų tik taisyklės. Čia — „kodėl", radiniai, versijų aprašymai,
„Kur dar yra tas pats?" lentelė. Skaitoma pagal poreikį: prieš keičiant sritį,
ieškok jos antraštės čia (`grep -n "## " docs/pamokos.md`).

Pasenę teiginiai šiame faile NEtaisomi (istorija). Galioja `CLAUDE.md`. Žinomi pasenę:
- „produkciją pasiekia tik naršyklė" — nuo 2026-09-21 yra Railway MCP (žurnalai, deploy'ai).
- „git vykdo TIK Lukas" — git vykdo Claude per Luko PowerShell, push tik Lukui leidus.
- `UZDUOTYS-DIZAINERIUI.md` ir „Patikrink žurnalą" — nuo v2 vietoj jų `BUSENA.md` + veidrodis + paruoštos žinutės.
- Klaidų būsenos `nepasitvirtino` / `atideta` — nuo v2.6.4 `neaktualu` / `laukia-sprendimo` / `laukia-dizainerio`.
...
| Matavimas ne toje būsenoje, kurioje gyvena klaida | `Z-50` – `scrollWidth` ant `overflow: visible`; `A-33` – `scrollWidth` ant `<select>`, kuris kerpa tekstą savo vidumi | `Z-59` – `K-34` matavau `.ct-shell-side`, kai `ct-shell` dar neturėjo `is-split`: šonas buvo 1240 px, ne 276, `overflow` – `visible`. Skaičius gavau, jis atrodė prasmingas, ir aš jį išsiunčiau dizaineriui. Bendra dalis su `A-33` ne ta, kad abu naudojom `scrollWidth`, o ta, kad **įrankis niekada neperspėja, jog klausimas jam netaikomas – jis tiesiog atsako**. **Prieš matuojant – įsitikinti, kad elementas YRA toje būsenoje, kurioje reiškiasi klaida** (klasė, medija, matomumas), ir tai užrašyti kartu su skaičiumi, v2.3.0 |
| Pasikartojanti atsvara = trūkstamas skyrius (ir pasikartojanti LAIKE) | – | `Z-50` – `ct-priedai.css` 13 ir 14 blokai rašė tą patį trejetą (`overflow/text-overflow/white-space`) dviem vardais, nes sistemoje nebuvo vienaeilio apkarpymo. Trūkstamo dalyko neranda joks skriptas – tikrinti nėra ko. **Antrą kartą rašant tą patį į `ct-priedai.css` – neberašyti, o klausti dizainerio, ar tam neturi būti klasės**, v1.99.0. `Z-59` priedas (dizainerio žodis): atsvara kartojasi ir LAIKE – 7 blokas (`.ct3-search-panel`, 2026-09) ir `K-34` (`.ct3-stats-bar`, po mėnesio) faile niekada nebuvo matomi kaip pora, juos sieja tik priežastis. **Rašant atsvarą – patikrinti, ar tos pačios priežasties nėra jau atsvertoje eilutėje kitam elementui** |
