...
## Z-143 · 2026-09-24 · Klaudijus · v2.16.0 – „Analizuoti modelį“

- **Luko prašymas:** po paieškos – mygtukas, kuris paima viską, ką turime (Wikipedia, Regitra, forumai, gamintojas) ir papasakoja apie patį modelį: rinkos tendencijas, kainų svyravimus, problemas.
- **Principas: skaičiai – mūsų, pasakojimas – AI.** Naujas `backend/modelio-apzvalga.js` surenka Regitros (parkas, importas, apyvarta, TA neišlaikiusių dalis, ridos normos), kartų lentelės ir mūsų sukauptos kainų istorijos (kryptis, pardavimo greitis) skaičius ir paduoda juos AI kaip faktus su nurodymu **neperskaičiuoti**. AI su `web_search` (5 užklausos) prideda tai, ko mūsų duomenyse nėra: kartas, variklius, tipines bėdas su šaltiniais, ką tikrinti, eksploataciją.
- **Kaina: 2 kreditai** (`planai.js KAINOS.modelioApzvalga`), bet **talpykla tikrinama PRIEŠ nurašymą**: apžvalga vienoda visiems, guli 30 d. (`MODELIO_PODELIS_D`), tad antras užsakovas gauna ją iš karto ir nemokamai. Mums vienas naujas tekstas kainuoja ~0,08 € (web paieška + tokenai).
- Nepavykus išanalizuoti atsakymo grąžinamas 500 – kreditai nurašomi atgal automatiškai (`reikalautiKreditu` sargas).
- **Priekinė dalis:** mygtukas rūšiavimo juostoje, matomas tik kai paieškoje buvo pasirinktas modelis. Langas – dizaino sistemos `.ct-modal`; skyrių stiliai kol kas inline (dizaino skola, pereis į dizainerio klases kartu su apžvalgos langu).
- Į apžvalgą keliauja ir ką tik matytos kainos (kiek rasta, mediana, intervalas), kad tekstas remtųsi tais pačiais skaičiais, kuriuos vartotojas mato ekrane.
- Naujas testas `modelio-apzvalga.test` (28/28) – duomenų rinkimas ir promptas tikrinami be tinklo ir be AI.
- **Kainų diagrama pagal metus** (Luko prašymas tą pačią dieną). Skaičiuojama naršyklėje iš TOS PAČIOS paieškos: kiekvienų gamybos metų kainos mediana, rodomi tik metai su bent 2 skelbimais (iš vienos kainos mediana nėra mediana). SVG stulpeliai ≤ 24 px, 4 px apvalus viršus, 2 px tarpas, plona pagrindo linija; skaičius užrašomas tik ties brangiausiais ir pigiausiais metais, likusius pasako užvedimas. Logika tikrinama testu su netikra paieška, ne tik „ar kodas yra“.
- **Pokalbis, ne pavieniai klausimai** (Luko prašymas: „kad ir AI naudotų kaip chat funkciją“). Kontekstas (duomenys + analizė) siunčiamas VIENĄ kartą pirmoje žinutėje, toliau eina pokalbio žinutės, todėl „o kaip dėl dyzelio?“ suprantama. Istorija apkarpoma serveryje: 8 paskutinės žinutės, po 1 200 ženklų – kitaip ilgas pokalbis brangtų tolydžio; naršyklė siunčia `pokalbis.slice(-8)`. Priimamos tik `user` ir `assistant` rolės, tekstas tikrinamas – istorija ateina iš naršyklės, tad ja nepasitikim aklai.
